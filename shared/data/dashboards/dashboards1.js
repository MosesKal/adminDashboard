...
const Statistics = () => {
    ...
    const [chartData, setChartData] = useState({
        series: [], options: {
            chart: {
                ...
                borderColor: "#f2f6f7", show: true,
            }, plotOptions: {
                bar: {
                    columnWidth: "60%", borderradius: "5px", colors: {
                        ranges: [{
                ...
                type: "month",
                categories: ['Janv.', 'Fév.', 'Mar.', 'Avr.', 'Mai', 'Juin', 'Juill.', 'Août', 'Sept.', 'Oct.', 'Nov.', 'Déc.',],
                axisBorder: {
                    show: true, color: "rgba(119, 119, 142, 0.05)", offsetX: 0, offsetY: 0,
                },
                axisTicks: {
                    show: true,
                    borderType: "solid",
    ...
    const processSolutions = (solutions) => {
        ...
        setChartData((prevData) => ({
            ...prevData, series: groupedData.series, options: {
                ...prevData.options, xaxis: {
                    ...prevData.options.xaxis, categories: groupedData.categories,
    ...
    const groupDataByMonthAndStatus = (data) => {
        ...
        const series = Object.keys(groupedData[Object.keys(groupedData)[0]]).map((status) => {
            return {
                name: status, data: Object.keys(groupedData).map((month) => groupedData[month][status] || 0),
            }
        });
    ...
    return (<div id="chart">
        {chartData.series.length === 0 ? (<div className="text-center">
