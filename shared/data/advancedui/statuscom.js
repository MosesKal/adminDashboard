...
import { columns as configureColumns } from "@/shared/data/advancedui/statusList";
import {
  Button,
  Card,
  Col,
...
const Settingscom = ({ updateRoles }) => {
  ...
  const handleEditRole = async (e) => {
    ...
    try {
      if (editedRoleName !== selectedRoleForEdit.name) {
        ...
        await axios.patch(`/status/${selectedRoleForEdit.id}`, updatedData);
        ...
        toast.error("Le nom du status n'a pas été modifié");
      }
    } catch (error) {
  ...
  const handleConfirmDelete = async (role) => {
    try {
      await axios.delete(`/status/${role.id}`);
