...
import { columns as configureColumns } from "@/shared/data/advancedui/challengsList";
import {
  Button,
  Card,
  Col,
...
const Settingscom = ({ updateRoles }) => {
  ...
  const handleEditRole = async (e) => {
    ...
    try {
      if (editedRoleName !== selectedRoleForEdit.name) {
        ...
        await axios.patch(`/challenges/${selectedRoleForEdit.id}`, updatedData);
  ...
  const handleConfirmDelete = async (role) => {
    try {
      await axios.delete(`/challenges/${role.id}`);
    ...
    <div>
      ...
      <Modal show={showEditModal} onHide={() => setShowEditModal(false)}>
        <Modal.Header className="modal-header">
          <h6 className="modal-title">{"Modifier le défi"}</h6>
          <Button
            variant=""
            className="btn-close"
            type="button"
