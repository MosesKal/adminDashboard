...
import { columns as configureColumns } from "@/shared/data/advancedui/callsList";
import {
  Button,
  Card,
  Col,
...
const Settingscom = ({ updateRoles }) => {
  ...
  const handleEditRole = async (e) => {
    ...
    try {
      if (editedRoleName !== selectedRoleForEdit.name) {
        ...
        await axios.patch(`/calls/${selectedRoleForEdit.id}`, updatedData);
  ...
  const handleConfirmDelete = async (role) => {
    try {
      await axios.delete(`/calls/${role.id}`);
    ...
    <div>
      ...
      <Modal show={showEditModal} onHide={() => setShowEditModal(false)}>
        ...
        <Modal.Body className="modal-body">
          ...
          {/* Vous pouvez utiliser les mêmes composants de formulaire que pour la création */}
        </Modal.Body>
        <Modal.Footer>
          <Button
            variant=""
