...
const Settings = () => {
  ...
  const updateRoles = async () => {
    try {
      const responseRoles = await axios.get("/status");
  ...
  const handleCreateRole = async (e) => {
    ...
    try {
      ...
      await axios.post("/status", JSON.stringify(payload));
      toast.success("Status créé avec succès !");
      handleClose();
          ...
          <div className="left-content mt-2">
            ...
            <Modal show={show} onHide={handleClose}>
              <Modal.Header className="modal-header">
                <h6 className="modal-title">{"Ajouter Status"}</h6>
                <Button
                    variant=""
                    className="btn-close"
                    type="button"
